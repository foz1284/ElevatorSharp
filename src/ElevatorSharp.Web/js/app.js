...
var createEditor = function() {
    ...
    var sm = CodeMirror.fromTextArea(document.getElementById("source"), {
        lineNumbers: true,
        indentUnit: 4,
        indentWithTabs: false,
        theme: "solarized light",
        ...
        extraKeys: {
            // the following Tab key mapping is from http://codemirror.net/doc/manual.html#keymaps
            Tab: function (sm) {
                var spaces = new Array(sm.getOption("indentUnit") + 1).join(" ");
                sm.replaceSelection(spaces);
            }
        }
    });

    // reindent on paste (adapted from https://github.com/ahuth/brackets-paste-and-indent/blob/master/main.js)
    sm.on("change", function (codeMirror, change) {
        if (change.origin !== "paste") {
            return;
        }
